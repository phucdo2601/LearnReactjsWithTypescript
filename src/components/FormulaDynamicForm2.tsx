...
const FormulaDynamicForm2 = () => {
  ...
  const handleChangeInput2 = (index: any, e: any) => {
    const values: any = [...inputFields];
    values[index]["valueNum"] = e.target.value;
    setInputFields(values);
  };
  ...
  const handleSubmit = (e: any) => {
    ...
    inputFields.map((e) => {
      ...
      if (t1 !== null) {
        console.log("Gia tri phan tu trong mang: ", t1);
      }
      formualString += " "+ e.valueChar + " " + e.valueNum + " ";
      listAllCharChoose.push(e.valueChar);
      listAllCharChoose.push(e.valueNum);

      if (
        e.valueChar.match(letterCharRegx) &&
        !listNormalCharChoose.includes(e.valueChar)
      ) {
        listNormalCharChoose.push(e.valueChar);
      }
    });
  ...
  const handleNormalCharChange = (index: any, e: any) => {
    ...
    values[index]["valueChar"] = e.target.value;
    setInputFields(values);
  };
        ...
        {isSendClicked && (
          <>
            ...
            {inputFields.map((model, index) => (
              <>
                <div>{model.valueChar}</div>
                <div>{model.valueNum}</div>
              </>
            ))}
...
export default FormulaDynamicForm2;
